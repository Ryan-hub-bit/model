...
directory = "/home/isec/Documents/data/NeuTyPro-O2-Set-1-01/binaries"
...
for accounts in os.listdir(directory):
    ...
    for root, dirs, files in os.walk(account):
        for binaries in files:
            ...
            jsonfile = os.path.join(account, binaries)
            # print(jsonfile)
            if os.path.isfile(jsonfile) and jsonfile.endswith(jsonex):
                ...
                def  getCallsiteAddress(callKey):
                    callsiteFile = os.path.join(account, binaryName +".txt")
                    if not os.path.exists(callsiteFile):
                        print(f"Err: {callsiteFile} not in {account}")
                    with open(callsiteFile, 'r') as f:
                        callsites =f.readlines()
                    callsites = [s[:-1] for s in callsites]
                    #print(callsites)
                    #print(callKey)
                    addressFileName = binaryName + "_address.txt"
                    addressFile = os.path.join(account, addressFileName)
                    if not os.path.exists(addressFile):
                        print(f"Err: {addressFile} not in {account}")
                ...
                for callKey in tdict['tg_targets']:
                    callsite_address_16 = getCallsiteAddress(callKey)
                    #print(f"callsite_address_16: {callsite_address_16}")
                    callsite_address = int(callsite_address_16, 16)
